...
class AddressBook(UserDict):
        ...
        for rec in self.data.values():
            if rec in self.data.values():
                if str(symb).lower() in str(rec.name).lower():
                    result.append(
                        [rec.name, rec.birthday, rec.mail, rec.address, ", ".join(p.value for p in rec.phones)])
                else:
    ...
    def show_rec(self, name):
        result = [name, self.data[name].birthday, self.data[name].mail, self.data[name].address,
...
class Phone(Field):
    ...
    def __init__(self, value):
        ...
        self.phone = PhoneVerification()
        self._value = self.phone.verificate(value)
...
class Birthday(datetime):
    ...
    def __init__(self, year, month, day):
        super().__init__()
        self.bday = BirthdayVerification()
        self.__birthday = self.bday.verificate(year, month, day)
...
class Mail(Field):
    def __init__(self, value):
        super().__init__(value)
        self.mail = MailVerification
        self.value = self.mail.verificate(value)
    ...
    @value.setter
    def value(self, value):
        self.__value = self.mail.verificate(value)
...
class Record:
    ...
    def add_user_birthday(self, year, month, day):
        self.birthday = self.bday.verificate(int(year), int(month), int(day))
    ...
    def days_to_birthday(self):
        ...
        if self.birthday is not None:
            ...
            current_year_birthday = datetime(current_year, birthday.month, birthday.day).date()
            delta = current_year_birthday - current_date
            if delta.days >= 0:
                return f"{self.name}'s birthday will be in {delta.days} days"
            else:
                next_year_birthday = datetime(current_year + 1, birthday.month, birthday.day).date()
