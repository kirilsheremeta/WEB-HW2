...
import os
import shutil
from pretty_view import SortDirectoryView
from verification import NameVerification
...
class SortDirectory:
    ...
    def sort_create_files(self, start_path):
        ...
        list_programming = ('PY', 'PHP', 'HTML', 'JS', 'CSS', 'CPP', 'CS')
        known_extensions = []
        unknown_extensions = []
            ...
            for file in files:
                try:
                    ...
                    if list_files[-1].upper() in list_image:
                        ...
                        filename = self.name.verificate(file)
        ...
        return print(self.view.create_row(result))

def run_sort():
    try:
        start_path = input('Input path: ')
        path = SortDirectory()
        path.sort_create_files(start_path)
        while True:
